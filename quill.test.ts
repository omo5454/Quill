...
function run(source: string): Map<string, any> {
...
describe("Lexer", () => {
  ...
  describe("keywords", () => {
    test("recognises 'let'", () => {
      ...
      expect(toks[0]).toMatchObject({ type: TokenType.Keyword, value: "let" });
    });
    ...
    test("recognises 'printf'", () => {
      expect(tokenize("printf")[0].type).toBe(TokenType.Keyword);
    });

    test("recognises 'say'", () => {
      expect(tokenize("say")[0].type).toBe(TokenType.Keyword);
    });
    ...
    test("recognises boolean literals True / False", () => {
      ...
      expect(toks[0]).toMatchObject({ type: TokenType.Keyword, value: "True" });
  ...
  describe("operators", () => {
    ...
    test.each(cases)("operator %s", (src: any, expected: any) => {
  ...
  describe("incrementation tokens", () => {
    ...
    test("i++ tokenizes identifier then incrementation", () => {
      ...
      expect(toks[0]).toMatchObject({ type: TokenType.Identifier, value: "i" });
      expect(toks[1]).toMatchObject({
        type: TokenType.Incrementation,
        value: "++",
      });
  ...
  describe("comments", () => {
    test("# comment token", () => {
      expect(tokenize("# hello")[0]).toMatchObject({ type: TokenType.Comment });
    });

    test("comment does not bleed into next line", () => {
      const toks = tokenize("# comment\nlet x = 1");
      expect(toks[0].type).toBe(TokenType.Comment);
      expect(toks[1]).toMatchObject({ type: TokenType.Keyword, value: "let" });
    });
  });
...
describe("Interpreter", () => {
  ...
  describe("comparisons", () => {
    ...
    test.each(cases)("%s", (src: any, expected: any) => {
  ...
  describe("conditionals", () => {
    ...
    test("else branch executes when condition false", () => {
      const vars = run(
        "let x = 0; if False { let x = 1; } else { let x = 2; }",
      );
      expect(vars.get("x")).toBe(2);
    });
  ...
  describe("standard library", () => {
    ...
    test("timeNow returns a non-empty string", () => {
      ...
      expect((vars.get("r") as string).length).toBeGreaterThan(0);
    });
